...
`/admin`. Adding a page means touching both files — the route alone
leaves it unreachable. **`/synergy` was retired on 13 Sep 2026** (the lead: "retire it"): the page was
linked from nowhere and its data empty; the Roster's Players view and Patterns' Starters filter answer
what it did, and `getTeamSynergy` went with it.
...
- **Sizing is in `rem`, not `px`.** The root font size follows the window (13 Sep 2026, the lead: Chrome's 175% on a 2560 screen should be the default at 100%): `clamp(1rem, min(100vw / 94, 100vh / 44), 3rem)`, so any window 1504px or wider is the layout of a 1504px window magnified to fill it — unchanged below that, 27px (70% larger) at 2560x1290, with the page column `min(85rem, 100% - 2rem)`, 90% of a wide screen. **Budget heights against about 47rem**: a 16:9 screen is 94rem wide and 44-50rem tall in rem, whatever its pixels. **Browser zoom does nothing between the floor and the cap** (zooming shrinks the window in CSS px by the same factor), and Chrome remembers a zoom per site, so someone who zoomed to try it must press Ctrl+0. **The film room keeps the old curve** (`html:has(.page.is-film)`): it already fills the screen, and a bigger root only shrank its Rift. **Verify at an emulated size** (Playwright viewport, or the Browser pane's resize_window) — DevTools docked on any side changes the window and so the whole scale. A `px` width or height silently opts out of the scale and will look wrong on a large screen. Borders, radii and shadows are the exception and should stay in `px`. Material Symbols carry an explicit `1.5rem` because Google's stylesheet pins them at 24px.
